  ...
  created: async function () {
    try {
      let response = await axios.get(
        `http://localhost:3030/admins/clients/${this.$route.params.id}`,
        this.axiosConfig
      );
      ...
      this.birth = response.data.birth.split("T")[0];
      this.sex = response.data.sex;
      response.data.Phones.forEach((phone) => {
        if (phone.type == 0) {
          this.ddPhone = phone.dd;
          this.numberPhone = phone.phoneNumber;
        } else {
          this.ddCellPhone = phone.dd;
          this.numberCellPhone = phone.phoneNumber;
        }
      });
  ...
  methods: {
    ...
    async update() {
      try {
        ...
        await axios.put(
          `http://localhost:3030/admins/clients/${this.id}`,
          {
            name: this.name,
            birth: this.birth,
            cpf: this.cpf,
